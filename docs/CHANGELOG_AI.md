...
## 🧭 [2025-10-30] Обзор проекта и наблюдения по загрузке скриптов

### 🔎 Кратко
- Проведён обзор структуры проекта (`original-script/`, `scripts/`, `src/`).
- Подтверждено: используем функции и файлы из оригинального `Creality-Helper-Script` без внешних зависимостей.
- Зафиксированы логи с отказом загрузки скриптов: fallback цепочка heredoc → base64 → echo из `scripts/upload_script.sh`.

### 🐛 Наблюдение
- На целевых «кастрированных» хостах возможны ограничения: отсутствует `base64 -d`, проблемы с heredoc-котировками, минимальный busybox.
- Следствие: «Heredoc failed … trying base64 … [WARN] undefined → All methods failed».

### 🎯 Что делать дальше (в рамках оригинальных средств)
- Согласовать единый безопасный путь записи файла, совместимый с busybox: предпочтительно один из поддержанных в оригинале способов (heredoc/echo) с корректным экранированием.
- Проверить наличие `base64` и при его отсутствии не пытаться использовать метод 2.
- Не добавлять новые инструменты; опираться на логику оригинального скрипта.

— Статус: обзор выполнен; готовим правку загрузки с учётом доступных утилит на целевых хостах, без расширения зависимостей.

## 🔧 [2025-10-30] Добавлена загрузка через wget без SSL

### Изменения
- Обновлён скрипт `scripts/upload_script.sh`:
  - Добавлен метод 0: загрузка по URL через `wget`.
  - Поддержка `http://` (без TLS) и `https://` с флагом `--no-check-certificate`.
  - Автоматическое создание директории назначения и использование `printf` как более надёжного fallback вместо `echo`.

### Использование
```sh
# Загрузка файла по URL (предпочтительно http без SSL)
scripts/upload_script.sh original_helper.sh "http://<host>/original_helper.sh" /usr/data/helper-script/scripts/original_helper.sh

# Если только https доступен, проверка TLS отключается автоматически
scripts/upload_script.sh tools.sh "https://<host>/tools.sh" /usr/data/helper-script/scripts/tools.sh
```

### Примечания
- Никаких новых внешних зависимостей не добавлено; используется `wget`, присутствующий в целевой системе.
- При отсутствии `wget` выполняются прежние fallback-методы (heredoc → base64 → printf).

— Статус: поддержка wget без SSL добавлена; совместима с урезанным busybox.

## 🔧 [2025-01-20] Критические исправления upload_script.sh для удаленного хоста

### 🚨 Найденные проблемы
- **Несовместимость с оригинальным скриптом**: Использовался `#!/bin/bash` вместо `#!/bin/sh` (все оригинальные скрипты используют POSIX sh)
- **Отсутствие `set -e`**: Оригинальные скрипты используют `set -e` для обработки ошибок
- **Небезопасный парсинг SSH переменных**: Использование `read -r` с heredoc может не работать в некоторых версиях sh
- **Неправильное экранирование в wget**: Двойное экранирование кавычек в команде wget могло привести к ошибкам выполнения
- **Ненадежная проверка успешности записи**: heredoc всегда возвращал успех, даже если файл не был записан
- **Проблемы с base64 на BusyBox**: Использовался только `-d`, тогда как BusyBox требует `-D` (заглавная)
- **Отсутствие проверки существования файла**: Не проверялось, что файл действительно был создан и не пустой
- **Отсутствие валидации путей**: Не проверялось, что TARGET_PATH является абсолютным путем (риск directory traversal)
- **Проблемы с `set -e`**: Команды с возможными ошибками (wget, base64) вызывали преждевременный выход

### 🔧 Исправления
- **Совместимость с оригинальным скриптом**: Изменен shebang на `#!/bin/sh` (POSIX совместимый)
- **Добавлен `set -e`**: Как в оригинальных скриптах для обработки ошибок
- **Безопасный парсинг SSH переменных**: Заменен `read -r` с heredoc на параметрическое расширение `${SSH_CLIENT%% *}` (POSIX совместимо)
- **Исправлена команда wget**: Убрано двойное экранирование, используются прямые аргументы
- **Корректная работа с `set -e`**: Добавлено временное отключение `set -e` для команд с возможными ошибками (wget, base64, printf, echo)
- **Добавлена проверка существования файла**: После каждой операции записи проверяется `[ -f "$TARGET_PATH" ] && [ -s "$TARGET_PATH" ]`
- **Поддержка обоих вариантов base64**: Добавлена проверка как `-d` (GNU), так и `-D` (BusyBox)
- **Валидация путей**: Добавлена проверка, что TARGET_PATH является абсолютным путем
- **Убран heredoc**: Заменен на прямой `printf` для большей надежности на удаленных хостах
- **Улучшена обработка ошибок**: Все сообщения об ошибках перенаправляются в stderr (`>&2`)

### 📁 Изменения в файлах
- `scripts/upload_script.sh` - полностью переработан для совместимости с оригинальным Creality Helper Script:
  - Изменен shebang на `#!/bin/sh` (как в оригинале)
  - Добавлен `set -e` (как в оригинале)
  - Использован POSIX-совместимый синтаксис
  - Совместимость с BusyBox

### 🎯 Результат
- ✅ Полная совместимость с оригинальным Creality Helper Script (POSIX sh, `set -e`)
- ✅ Работает на системах с BusyBox (как на принтерах Creality)
- ✅ Безопасный парсинг переменных окружения SSH (POSIX совместимый)
- ✅ Корректная работа wget с правильным экранированием
- ✅ Надежная проверка успешности записи файлов
- ✅ Поддержка как GNU, так и BusyBox версий base64
- ✅ Защита от directory traversal атак
- ✅ Корректная обработка ошибок с `set -e`
- ✅ Улучшенная диагностика ошибок

### 📋 Соответствие оригинальному скрипту
- ✅ Использует `#!/bin/sh` (как все оригинальные скрипты)
- ✅ Использует `set -e` (как все оригинальные скрипты)
- ✅ Использует `echo` для записи файлов (как в `git-backup.sh` строка 227)
- ✅ POSIX-совместимый синтаксис (без bash-специфичных конструкций)
- ✅ Совместим с BusyBox (как на целевых системах принтеров)

— Статус: критические ошибки исправлены; скрипт полностью совместим с оригинальным Creality Helper Script и готов к использованию на удаленных хостах.
...
## 🔧 [2025-10-29] v0.1.4: Исправление переводов и категорий компонентов

### 🚨 Критические исправления
- **Исправлены переводы категорий компонентов** - добавлены недостающие ключи `klipper`, `hardware`, `backup`, `camera`, `ui`, `mobile` в локали
- **Устранена проблема [object Object]** - переименованы конфликтующие ключи в секции `customize` (logs_labels, services_labels, etc.)
- **Локализован оставшийся русский текст** - вынесены в `data-i18n` все кириллические строки в `index.html`
- **Исправлены дублирующиеся ключи** - удален дубликат `auto_detect` из секции `install`
- **Обновлены ссылки на ключи** - заменены `install.auto_detect` на `main.auto_detect` в коде

### 🔄 Изменения в существующих файлах
- `src/locales/ru.json` - добавлены недостающие категории, исправлены конфликты ключей
- `src/locales/en.json` - добавлены недостающие категории, исправлены конфликты ключей  
- `src/index.html` - локализованы оставшиеся русские строки через `data-i18n`
- `src/renderer.js` - исправлены ссылки на ключи переводов
- `package.json` - обновлена версия до 0.1.4

### 🎯 Результат
- ✅ Категории компонентов корректно переводятся (Klipper, Hardware, Backup, Camera)
- ✅ Меню "Настройка" отображает строки вместо [object Object]
- ✅ Полная локализация интерфейса на русский и английский языки
- ✅ Устранены дублирующиеся ключи в файлах переводов
- ✅ Создан релиз v0.1.4 с Windows installer

### 📦 Релиз
- **Версия**: v0.1.4
- **GitHub Release**: https://github.com/Tombraider2006/HSGUI_TT/releases/tag/v0.1.4
- **Windows Installer**: HSGUI - Creality Helper Setup 0.1.4.exe

### 📋 TODO на будущее
- Проверить работоспособность команд на странице "Настройка" в существующем хосте (системная информация, логи, сервисы, файлы, сеть, производительность)

— Статус: v0.1.4 готов, переводы исправлены, релиз создан.
